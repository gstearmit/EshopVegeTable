...
class Rohlik {
	...
	protected $host; // https://www.rohlik.cz
	...
	public function _is_checkurl($url) {
		...
		curl_close ( $ch );
		$this->setStatus_url_check($httpcode);
		unset($ch, $options);
		//return $httpcode; // return ($httpcode >= 200 && $httpcode < 300);
	}
	...
	public function getProductName(Query $dom, $_feed_dom = null) {
		...
		if (count ( $get_Product_Name ) > 0) {
			...
			$this->setPropertyProductName("loi khi lay product name and  product url and data_popuplink");
		}
		
		$this->setPropertyProductName($_ProductName);
		$this->setPropertyProduct_Url($_Product_Url);
		$this->setPropertydata_popuplink($_data_popuplink);
		$this->setPropertyProductId($_Product_id);
	}
	...
	public function getProductImg(Query $dom, $_feed_dom = null) {
		...
		if (count ( $get_Product_Img ) > 0) {
			...
			$this->setPropertyProductImg("loi khi lay product img");
		}
		
		$this->setPropertyProductImg($_ProductImg);
	}
	public function getProductPrice(Query $dom, $_feed_dom = null) {
		...
		if (count ( $get_Product_Price ) > 0) {
			...
			$this->setPropertyProductPrice("loi khi lay product Price");
		}
		
		$this->setPropertyProductPrice($_ProductPrice);
	}
	...
	public function feed_all($url = null, $_dom_name = null, $_dom_img = null, $_dom_price = null, $_dom_promotion = null,$_dom_countryoforigin=null,$_dom_detail_product_descriptstion=null) {
		
		if ($url == null) return false;
		$this->_is_checkurl ( $url );
		$url_check = $this->getStatus_url_check();           
		if ($url_check != 200) return "Url is error : " . $url_check; 
		$this->setPropertyUrl($url);
 		$this->getBodyUrl($url); 
 		$_body =  $this->getBody();
		$dom = new Query ($_body);
 		$this->getProductName ( $dom, $_dom_name ); // $this->ProductName = $_ProductName; $this->Product_Url = $_Product_Url;
		$this->getProductImg ( $dom, $_dom_img );
 		$this->getProductPrice ( $dom, $_dom_price );
		$this->getProductPromotion ( $dom, $_dom_promotion );
		...
		$this->getProductDetails_by_Product_Url($_dom_countryoforigin1,$_dom_detail_product_descriptstion2);
		
 		$this->Exchangdata(); 
 	//	$this->getDetail($_dom_countryoforigin,$_dom_detail_product_descriptstion);
	}
	
	public function getProductDetails_by_Product_Url($_dom_countryoforigin,$_dom_detail_product_descriptstion)
	{
		$_PrDuct_Url = $this->getPropertyProduct_Url();
		$_ProductDetails = array(); $_CountryofOrigi= array(); $_HttpCode = array(); $_Url_get = array(); $_Body_get = array();
		 $value_url_test = 'https://www.rohlik.cz/1295045-gambrinus-original-10-6x-500ml-6ks';
		$host_tmp = $this->getHost(); 
...
// 			   $_CountryofOrigi[$key] = $PropertyCountryofOrigin[0];
			...
			//set bien o doi tuong nay nua de lay ra ngoai
// 			$this->setPropertyProductDetail($_ProductDetails);
// 			$this->setPropertyCountryofOrigin($_CountryofOrigi);
			$this->setPropertyHttpCode($_HttpCode);
			$this->setPropertyUrl_get($_Url_get);
			$this->setPropertyBody_get($_Body_get);
		}
	}
	
	public function Exchangdata() {
		$napoje_tmp = array ();
		$_ProdName = $this->getPropertyProductName();
		$_ProdUrl = $this->getPropertyProduct_Url();
		$_ProdImg = $this->getPropertyProductImg();
		$_ProdPrice = $this->getPropertyProductPrice();
		$_ProdPromotion = $this->getPropertyProductPromotion();
		$_ProdData_popuplink = $this->getPropertydata_popuplink();
		$_host_tmp = $this->getHost();
		$_url = $this->getPropertyUrl();
		$_ProdId = $this->getPropertyProductId();
		$_ProdDetail = $this->getPropertyProductDetail();
		$_ProdCountryofOrigin = $this->getPropertyCountryofOrigin();
		$_ProdHttpCode_details = $this->getPropertyHttpCode();
		...
		$this->setPropertyIteams($napoje_tmp) ; // luu vao bien chung $Iteams
	}
