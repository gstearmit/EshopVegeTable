...
namespace Masterialproduct\Controller;

use Zend\Mvc\Controller\AbstractActionController;
use Zend\View\Model\ViewModel;
use Masterialproduct\Form\MasterialproductFilter;
use Masterialproduct\Form\MasterialproductForm;
use Masterialproduct\Model\Masterialproduct;
use Masterialproduct\Model\MasterialproductTable;
use Masterialproduct\Model\Payoutype;
use Masterialproduct\Model\PayoutypeTable;
...
class IndexController extends AbstractActionController {
    ...
    public function viewAction() {
        ...
        if ($id != 0) {
            $product_all = $this->getServiceLocator()->get('MasterialproductTable')->get($id);
        } else
            die("Error , Masterialproduct !!!");
    ...
    public function indexAction() {
        ...
        $MasterialproductTable = $this->getServiceLocator()->get('MasterialproductTable');
        $allRecord = $MasterialproductTable->countAll();
        ...
        $listpr_tmp = $MasterialproductTable->getList($offset, $itemsPerPage);
            $i=0; $listpr = array();
    ...
    public function addalbumAction() {
        ...
        if ($getuser && ($getuser->group == 'admin' || $getuser->group == 'supperadmin')) {
            ...
            $form = new MasterialproductForm($dbAdapter);
            //$form->setInputFilter ( new MasterialproductFilter () );
            ...
            if ($request->isPost()) {
                ...
                if (!$form->isValid()) {
                    if ($data ['img'] ['name'] != '') {
                        ...
                        $check = $this->getServiceLocator()->get('MasterialproductTable')->save($product);
                        if ($check != 0) {
                        ...
                        if ($check != 0) {
                            $_url = WEBPATH . '/product/index/index';
                            $this->redirect()->toRoute('Masterialproduct', array(
                                'controller' => 'index',
                                'action' => 'add',
                                'id' => $check
                            ));
            ...
            $catalog_cr = $this->getServiceLocator()->get('MasterialproductTable')->get($product_id);
            
             $form->settagsId($tags);
            $Tags_cr = $this->getServiceLocator()->get('ProductTable')->get($product_id);
            $form->bind($Tags_cr);
            if ($product_id == 0) {
                $form->get('submit')->setAttribute('value', 'Add Masterialproduct');
            } else {
                $form->get('submit')->setAttribute('value', 'Edit Masterialproduct');
            ...
            $view->setVariable('Masterialproductform', $form);
    ...
    public function ajaxsaveimgAction() {
        ...
        if ($img == 1 and $product_id != 0) {
            //update img
            $_Masterialproduct_all = $this->getServiceLocator()->get('MasterialproductTable')->get($product_id);

            if ($_Masterialproduct_all->array_img != '' || $_Masterialproduct_all->array_img != null || $_Masterialproduct_all->array_img != NULL || $_Masterialproduct_all->array_img != (NULL)) {
                $array_img = @explode(',', $_Masterialproduct_all->array_img);
                for ($k = 0; $k <= count($array_img); $k++) {
                    // update img all
                    $check = $this->getServiceLocator()->get('MasterialproductTable')->saveimg($array_img[$k], $k, $product_id);
                ...
                //echo "Masterialproduct_all : ";echo "<pre>";print_r($_Masterialproduct_all);echo "</pre>";
                die("Error ! try agian!");
            }
        }

        if ($img == 0 and $product_id != 0) {
            $get_all_img = $this->getServiceLocator()->get('MasterialproductTable')->get($product_id);
            if ($get_all_img->array_img != null) {
                $img_array_ck = $this->getServiceLocator()->get('MasterialproductTable')->update_array_img($product_id);
    ...
    public function dragdropAction() {
        ...
        $_MasterialproductTable = $this->getServiceLocator()->get('MasterialproductTable');

        if (isset($_FILES ["myfile"])) {
            $ret = array();
            $tmp_Masterialproduct_all = ''; //
            ...
            if (!@is_array($_FILES ["myfile"] ['name'])) {     // single file
                ...
                $utility->createImageThumbnail($dirFileName, $dir . '\thumb_', $options);

                if ($product_id != 0) {
                    $_Masterialproduct_all = $_MasterialproductTable->get($product_id);
                    if ($_Masterialproduct_all->array_img != '' || $_Masterialproduct_all->array_img === null || $_Masterialproduct_all->array_img === NULL || $_Masterialproduct_all->array_img === '(NULL)') {
                        if ($_Masterialproduct_all->array_img === null) {
                            $tmp_Masterialproduct_all = $NewImageName;
                        } else
                            $tmp_Masterialproduct_all = $_Masterialproduct_all->array_img . ',' . $NewImageName;
                    }
                    //update array_img
                    $data_update_img = array(
                        'id' => $product_id,
                        'array_img' => $tmp_Masterialproduct_all,
                    );

                    $Masterialproduct = new Masterialproduct();
                    $Masterialproduct->exchangeArray($data_update_img);
                    $_check = $_MasterialproductTable->update_array_img_insert($Masterialproduct);
                }
                ...
                if ($form->isValid()) {
                    ...
                    if ($data ['img'] ['name'] != '') {
                        ...
                        if ($check != 0) {
                            ...
                            //$_url = WEBPATH . '/masteproduct/index/index';
                           // $this->redirect()->toUrl($_url);
                            $error='Add Product Sucess';
                        ...
                        $_url = WEBPATH . '/masteproduct/index/add' . $id;
                        echo '11111111122222222222';
                        if ($check == 0) {
                          
                            // notupdate
                            //$_url = WEBPATH . '/masteproduct/index/index';
                            $_url = WEBPATH . '/masteproduct/index/add';
                            //$this->redirect()->toUrl($_url);
                           $error='Add Product Sucess';
                            $view->setVariable('error', $error);
                    ...
                    $error='Full enter data. Do not leave empty fields with an asterisk';
                    $view->setVariable('error', $error);
                }
            }// end post
...
// 				$view->setVariable ( 'img_thumb0', $_img_thumb0 );
    ...
    public function listAction() {
        ...
        $listpr = $MasterialproductTable->getList($offset, $itemsPerPage);
            ...
            if ($request->isPost()) {
                ...
                if (!$form->isValid()) {
                    if ($data ['img'] ['name'] != '') {
                        ...
                        $product->dataArraySwap($form->getData(), $get_img_older);
                        $check = $this->getServiceLocator()->get('MasterialproductTable')->save($product);
                        $_url = WEBPATH . '/masteproduct/index/edit/' . $id;
                        if ($check == 0) {
                            // notupdate
                            $_url = WEBPATH . '/masteproduct/index/index';
                            $this->redirect()->toUrl($_url);
...
//                            ));
    ...
    public function statusAction() {
       ...
       if($status =='1'){
        ...
        if ($id == 0) {
            return $this->redirect()->toRoute('Masterialproduct', array(
                        'controller' => 'product',
                        'action' => 'list-news'
            ));
        } else {
           ...
           // print_r($exchange_data);die;
            $Masterialproduct = new Masterialproduct ();
            $Masterialproduct->exchangeArray($exchange_data);
           
            $checkupdate = $MasterialproductTable->savestatus($Masterialproduct);
