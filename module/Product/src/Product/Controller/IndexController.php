  ...
  //      $listpr = $ProductTable->getList ( $offset, $itemsPerPage );
            ...
            if ($request->isPost()) {
                ...
                if (!$form->isValid()) {
                    if ($data ['img'] ['name'] != '') {
                        ...
                        if ($check != 0) {
                            $_url = WEBPATH . '/product/index/index';
                            $this->redirect()->toRoute('Product', array(
                                'controller' => 'index',
                                'action' => 'add',
                                'id' => $check
                            ));
    ...
    public function ajaxsaveimgAction() {
        ...
        if ($img == 1 and $product_id != 0) {
            //update img
            $_Product_all = $this->getServiceLocator()->get('ProductTable')->get($product_id);
           // print_r($_Product_all);
            if ($_Product_all->array_img != '' || $_Product_all->array_img != null || $_Product_all->array_img != NULL || $_Product_all->array_img != (NULL)) {
	            $array_img = @explode(',', $_Product_all->array_img);
	                if(is_array($array_img)){
	                //	echo "hoang phuc".$product_id;die;
	                for ($k = 0; $k <= count($array_img); $k++) {
	                    // update img all
	                    $check = $this->getServiceLocator()->get('ProductTable')->saveimg($array_img[$k], $k, $product_id);
	                    $ret[$k][$array_img[$k]] = $check;
	                }
	
	                echo @json_encode($ret); die;
                  }else {die(" array_img is null error!");}
            } else {
                //echo "Product_all : ";echo "<pre>";print_r($_Product_all);echo "</pre>";
                die("Error ! try agian!");
            }
        }
    ...
    public function dragdropAction() {
        ...
        if (isset($_FILES ["myfile"])) {
            ...
            if (!@is_array($_FILES ["myfile"] ['name'])) {     // single file
                ...
                if ($product_id != 0) {
                    $_Product_all = $_ProductTable->get($product_id);
                    if ($_Product_all->array_img != '' || $_Product_all->array_img === null || $_Product_all->array_img === NULL || $_Product_all->array_img === '(NULL)') {
                        if ($_Product_all->array_img === null) {
                            $tmp_Product_all = $NewImageName;
                        } else
                            $tmp_Product_all = $_Product_all->array_img . ',' . $NewImageName;
                    }
                    //update array_img
                    $data_update_img = array(
                        'id' => $product_id,
                        'array_img' => $tmp_Product_all,
                    );

                    $Product = new Product();
                    $Product->exchangeArray($data_update_img);
                    $_check = $_ProductTable->update_array_img_insert($Product);
                }
