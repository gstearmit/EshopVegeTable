...
function delete_declaration_site(){
	if(check_selected()){
		...
		alert('Choose at least one record - Hãy chọn ít nhất một bản ghi');
		return false;
	}
}
function get_form()
...
// kich vao lay tin
function feed_data(){
	console.log("check_selected() :" + check_selected());//return 0;
	if(check_selected()){
		var form = get_form();
		jQuery('.template-data').hide();
		jQuery('.loading').show();
		jQuery('#cmd').val('feed'); 
		console.log("cmd  :" + jQuery('#cmd').val());
		form.submit();
	}else{
		jQuery('.loading').hide();
		alert('Choose at least one record - Hãy chọn ít nhất một bản ghi');
...
//feed_data_delete()
function feed_data_delete(){
	console.log("check_selected() :" + check_selected());//return 0;
	if(check_selected()){ 
		var r = confirm("Are you sure that you want to delete ? Note: If you delete this list, it will affect the list containing it. Or do you think more carefully before deleting");
		if (r == true) {
			var form = get_form();
			jQuery('.template-data').hide();
			jQuery('.loading').show();
			jQuery('#cmd').val('feed_delete'); 
			console.log("cmd  :" + jQuery('#cmd').val());
			form.submit();
		} else {
		    return false;
		...
		jQuery('.loading').hide();
		alert('Choose at least one record delete- Hãy chọn ít nhất một bản ghi de xoa');
		return false;
	}
}

//
function feed_data_detail(){
	console.log("feed_data");
     	var form = get_form();
		jQuery('.template-data').hide();
		jQuery('.loading').show();
		jQuery('#cmd').val('feeddetails'); 
		form.submit(); 
}
