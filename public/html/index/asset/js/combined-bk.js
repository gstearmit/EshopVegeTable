...
/*	var _dummyConfirmPrimaryEmail = ko.observable();
    ...
    self.checkout = function() {
		...
		$.postJSON('/ajax.php', { action: 'apply', status: 'pay later', tracking_id: self.tracking_id(), persons:data_persons, payment_type: self.payment_offline_method() }, function(res) {
			self.currentStep(3);
		});
    };
	...
	self.checkoutOnepay = function() {
		$.postJSON('/ajax.php', { action: 'onepay', tracking_id: self.tracking_id(), total: self.invoice.total() }, function(res) {
...
var VisaModelOld = function() {
	...
	self.custom = {
		'Mauritius': {'1ms':25, '1mm':30, '1mm_full':30, '3ms':38, '3mm':48},
		'Nepal': {'1ms':25, '1mm':30, '1mm_full':30, '3ms':38, '3mm':48},
		'India': {'1ms':25, '1mm':30, '1mm_full':30, '3ms':38, '3mm':'N/A'},
		'South Korea': {'1ms':25, '1mm':30, '1mm_full':30, '3ms':38, '3mm':48}
	};
	self.customHard = {
		'Nigeria': {'1ms':189}
	};
...
/*	self.info.visa_type.subscribe(function(newValue) {
    ...
    self.checkout = function() {
		if (self.payment_type().length > 0 && self.enable_payment_other()) {
			...
			var data_persons = self.persons().slice(0,self.persons().length);
			$.postJSON('/ajax.php', { action: 'apply', status: 'pay later', tracking_id: self.tracking_id(), persons:data_persons, payment_type: self.payment_type() }, function(res) {
				self.currentStep(3);
			});
		}
    };
